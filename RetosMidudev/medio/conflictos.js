...
function resolverConflictos(firstUserChanges, secondUserChanges) {
  const changes = [...firstUserChanges, ...secondUserChanges];
  let result = [];
  ...
  for (const x of changes) {
    ...
    if (x.op === "insert") {
      if (x.index >= 0 && x.index <= totalCount) {
        result.splice(x.index, 0, ...x.text);
      }
    } else {
      if (x.index >= 0 && x.index < totalCount) {
        result.splice(x.index, 1);
      }
    }
  }
...
const result = resolverConflictos(cambiosA, cambiosB);
console.log(result);
// => "HolaMundo cruelmundo"
